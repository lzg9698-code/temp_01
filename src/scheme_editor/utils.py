...
class TemplateUtils:
    ...
    @staticmethod
    def extract_variables(template_content: str) -> set[str]:
        """
        从Jinja2模板内容中提取所有变量名。
        使用 jinja2.Environment.parse 进行解析，比正则更准确。
        """
        try:
            ...
            # 过滤掉 Jinja2 的内置函数和变量
            # range, int, float, str, list, dict, set, bool
            # cycle, join, length, lower, upper, truncate, round, etc.
            # 这些通常不应该被识别为“缺失参数”
            builtin_names = {
                'range', 'int', 'float', 'str', 'list', 'dict', 'set', 'bool',
                'cycle', 'join', 'length', 'lower', 'upper', 'truncate', 'round',
                'abs', 'sum', 'min', 'max', 'sort', 'first', 'last', 'random',
                'loop', 'self', 'g', 'request', 'session', 'config', 'namespace'
            }
            
            # 过滤掉这些内置名称
            variables = {v for v in variables if v not in builtin_names}
            ...
            import re
            # 排除掉常见的关键字
            keywords = {'if', 'else', 'elif', 'endif', 'for', 'in', 'endfor', 'set', 'macro', 'endmacro', 'include', 'import', 'extends', 'block', 'endblock'}
            
            matches = re.findall(r"\{\{\s*([a-zA-Z_][a-zA-Z0-9_]*)\s*.*?\}\}", template_content)
            matches_ctrl = re.findall(r"\{%.*?\b([a-zA-Z_][a-zA-Z0-9_]*)\b.*?%\}", template_content)
            
            all_found = set(matches) | set(matches_ctrl)
            return {v for v in all_found if v not in keywords}
