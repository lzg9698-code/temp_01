...
class EditableScheme:
    ...
    def get_dict(self) -> Dict[str, Any]:
        ...
        # 如果有内建参数组，则添加 parameters 字段
        if self.parameter_groups:
            result["parameters"] = {g.name: g.get_dict() for g in self.parameter_groups}

        # 添加引用参数组
        if self.referenced_groups:
            result["referenced_groups"] = self.referenced_groups

        # 添加默认值
        final_defaults = self.defaults.copy()
        ...
        if final_defaults:
            result["defaults"] = final_defaults

        # 添加宏
        if self.macros:
            result["macros"] = self.macros
